...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, activation='relu', bn_momentum=0.01):
        ...
        self.convlayer1 = nn.Conv1d(in_channels=self.in_channels, out_channels=self.out_channels,
                                    kernel_size=kernel_size, dilation=dilation, stride=1, padding=paddingAmount,
                                    padding_mode='zeros')
        self.convlayer2 = nn.Conv1d(in_channels=self.in_channels, out_channels=self.out_channels,
                                    kernel_size=kernel_size, dilation=dilation, stride=1, padding=paddingAmount,
                                    padding_mode='zeros')
...
class SpliceAI(nn.Module):
    def __init__(self, CL_max, bn_momentum=0.01, **kwargs):
        ...
        self.kernel_size = 1
        self.conv_layer_1 = nn.Conv1d(in_channels=4, out_channels=self.n_channels, kernel_size=self.kernel_size,
                                      stride=1)
        self.skip_layers = nn.ModuleList([nn.Conv1d(in_channels=self.n_channels, out_channels=self.n_channels,
                                                    kernel_size=self.kernel_size, stride=1) for i in range(5)])
        self.res_layers = nn.ModuleList([ResComboBlock(in_channels=self.n_channels, out_channels=self.n_channels,
                                                       res_W=self.res_W[i], res_dilation=res_dilation[i],
                                                       bn_momentum=bn_momentum) for i in range(4)])
...
class SpliceFormer(nn.Module):
    def __init__(self, CL_max, n_channels=32, maxSeqLength=4 * 128, depth=4, n_transformer_blocks=2, heads=4,
                 dim_head=32, mlp_dim=512, dropout=0.01, returnFmap=False, bn_momentum=0.01, determenistic=False,
                 crop=True, **kwargs):
        super().__init__()
        self.n_channels = n_channels
        self.CL_max = CL_max
        ...
        self.skip_layers = nn.ModuleList([nn.Conv1d(in_channels=self.n_channels, out_channels=self.n_channels,
                                                    kernel_size=self.kernel_size, stride=1) for i in
                                          range(n_transformer_blocks + 1)])
    ...
    def select_action(self, state):
        ...
        if self.determenistic:
            ...
            acceptor_log_probs, acceptor_actions = torch.topk(acceptor_logits, self.maxSeqLength // 2, dim=1,
                                                              largest=True, sorted=False)
            donor_log_probs, donor_actions = torch.topk(donor_logits, self.maxSeqLength // 2, dim=1, largest=True,
                                                        sorted=False)
            ...
            for i in range(self.maxSeqLength // 2):
                m = torch.distributions.Categorical(
                    logits=policy_logits[:, :, 0] - torch.nan_to_num(float('inf') * c, nan=0))
                action = m.sample()
                ...
                m = torch.distributions.Categorical(
                    logits=policy_logits[:, :, 1] - torch.nan_to_num(float('inf') * c, nan=0))
                action = m.sample()
            ...
            return torch.hstack(actions).unsqueeze(2).repeat(1, 1, self.n_channels), torch.hstack(
                acceptor_actions), torch.hstack(donor_actions), torch.hstack(acceptor_log_probs), torch.hstack(
                donor_log_probs)
    ...
    def __init__(self, CL_max, **kwargs):
        ...
        self.skip_layers = nn.ModuleList(
            [nn.Conv1d(in_channels=n_channels, out_channels=n_channels, kernel_size=self.kernel_size, stride=1) for i in
             range(5)])
        self.res_layers = nn.ModuleList([ResComboBlock(in_channels=n_channels, out_channels=n_channels,
                                                       res_W=self.res_W[i], res_dilation=res_dilation[i]) for i in
                                         range(4)])
